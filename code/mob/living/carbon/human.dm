	...
	if(src.lying)
		if(src.wear_suit)
			if(src.wear_suit.track_spreader?.is_tracking())
				src.wear_suit.track_spreader.place_track(NewLoc, src.last_move, src.get_footprints())
		else if(src.w_uniform)
			if(src.w_uniform.track_spreader?.is_tracking())
				src.w_uniform.track_spreader.place_track(NewLoc, src.last_move, src.get_footprints())
	else if(src.shoes)
		if(src.shoes.track_spreader?.is_tracking())
			src.shoes.track_spreader.place_track(NewLoc, src.last_move, src.get_footprints())
	else if(src.limbs)
		if(src.limbs.r_leg?.track_spreader?.is_tracking())
			src.limbs.r_leg.track_spreader.place_track(NewLoc, src.last_move, src.get_footprints())
		if(src.limbs.l_leg?.track_spreader?.is_tracking())
			src.limbs.l_leg.track_spreader.place_track(NewLoc, src.last_move, src.get_footprints())
...
/mob/living/carbon/human/clean_forensic()
	...
	src.w_uniform?.clean_forensic()
	src.belt?.clean_forensic()
	src.glasses?.clean_forensic()
	src.wear_id?.clean_forensic()
	if(src.gloves)
		src.gloves.clean_forensic()
	else if(src.limbs)
		src.limbs.r_arm?.clean_forensic()
		src.limbs.l_arm?.clean_forensic()
	if(src.shoes)
		src.shoes.clean_forensic()
	else if(src.limbs)
		src.limbs.r_leg?.clean_forensic()
		src.limbs.l_leg?.clean_forensic()
	if(src.organHolder)
		src.organHolder.head?.clean_forensic()
		src.organHolder.chest?.clean_forensic()
		src.organHolder.butt?.clean_forensic()
	..()
...
/mob/living/carbon/human/proc/apply_evidence_clothing(var/datum/forensic_data/basic/f_data, var/category, var/include_body = TRUE)
	src.head?.add_evidence(f_data.get_copy(), category)
	if(src.gloves)
		src.gloves.add_evidence(f_data.get_copy(), category)
	else if(include_body && src.limbs)
		src.limbs.r_arm?.add_evidence(f_data.get_copy(), category)
		src.limbs.l_arm?.add_evidence(f_data.get_copy(), category)
	...
	else if(include_body && src.organHolder)
		src.organHolder.chest?.add_evidence(f_data.get_copy(), category)
		src.organHolder.butt?.add_evidence(f_data.get_copy(), category)
