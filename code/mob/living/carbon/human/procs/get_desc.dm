...
/mob/living/carbon/human/get_desc()
	...
	if (src.w_uniform && !(src.wear_suit?.hides_from_examine & C_UNIFORM))
		. += "<br><span class='[src.w_uniform.is_stained ? "alert" : "notice"]'>[src.name] is wearing [bicon(src.w_uniform)] \an [src.w_uniform.name].</span>"

	if (src.wear_suit)
		. += "<br><span class='[src.wear_suit.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.wear_suit)] \an [src.wear_suit.name] on.</span>"
	...
	if (src.ears && !(src.wear_suit?.hides_from_examine & C_EARS) && !(src.head?.hides_from_examine & C_EARS))
		if (istype(src.ears, /obj/item/clothing/))
			. += "<br><span class='[src.ears.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.ears)] \an [src.ears.name] by [t_his] mouth.</span>"
		else
			. += "<br><span class='[src.ears.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.ears)] [src.ears.is_stained ? "a bloody [src.ears.name]" : "\an [src.ears.name]"] by [t_his] mouth.</span>"

	if (src.head)
		. += "<br><span class='[src.head.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.head)] \an [src.head.name] on [t_his] head.</span>"
	...
	if (src.wear_mask && !(src.wear_suit?.hides_from_examine & C_MASK) && !(src.head?.hides_from_examine & C_MASK))
		if (istype(src.l_hand, /obj/item/clothing/))
			. += "<br><span class='[src.wear_mask.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.wear_mask)] [src.wear_mask.is_stained ? "a bloody [src.wear_mask.name]" : "\an [src.wear_mask.name]"] on [t_his] face.</span>"
		else
			. += "<br><span class='[src.wear_mask.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.wear_mask)] \an [src.wear_mask.name] on [t_his] face.</span>"

	if (src.glasses && !(src.wear_suit?.hides_from_examine & C_GLASSES) && !(src.head?.hides_from_examine & C_GLASSES))
		if (face_visible())
			. += "<br><span class='[src.glasses.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.glasses)] \an [src.glasses.name] on [t_his] face.</span>"

	if (src.l_hand)
		if (istype(src.l_hand, /obj/item/clothing/))
			. += "<br><span class='[src.l_hand.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.l_hand)] \an [src.l_hand.name] in [t_his] left hand.</span>"
		else
			. += "<br><span class='[src.l_hand.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.l_hand)] [src.l_hand.is_stained ? "a bloody [src.l_hand.name]" : "\an [src.l_hand.name]"] in [t_his] left hand.</span>"

	if (src.r_hand)
		if (istype(src.r_hand, /obj/item/clothing/))
			. += "<br><span class='[src.r_hand.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.r_hand)] \an [src.r_hand.name] in [t_his] right hand.</span>"
		else
			. += "<br><span class='[src.r_hand.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.r_hand)] [src.r_hand.is_stained ? "a bloody [src.r_hand.name]" : "\an [src.r_hand.name]"] in [t_his] right hand.</span>"

	if (src.belt)
		. += "<br><span class='[src.belt.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.belt)] [src.belt.is_stained ? "a bloody [src.belt.name]" : "\an [src.belt.name]"] on [t_his] belt.</span>"

	if (src.gloves && !src.gloves.nodescripition)
		if(!(src.wear_suit && src.wear_suit?.hides_from_examine & C_GLOVES))
			. += "<br><span class='[src.gloves.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.gloves)] [src.gloves.name] on [t_his] hands.</span>"
	else if (src.l_hand?.is_stained || src.r_hand?.is_stained)
	...
	if (src.back)
		. += "<br><span class='[src.back.is_stained ? "alert" : "notice"]'>[src.name] has [bicon(src.back)] [src.back.is_stained ? "a bloody [src.back.name]" : "\an [src.back.name]"] on [t_his] back.</span>"
