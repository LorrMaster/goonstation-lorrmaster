...
/datum/mutantrace/pug
	...
	proc/sleuth()
		...
		var/sleuth_text = "" // boutput(src.mob, SPAN_NOTICE(sleuth_text))
		if (ismob(A))
			var/mob/living/M = A
			if (M.mind)
				sleuth_text += SPAN_NOTICE("<li>[M] mostly smells like a [M.mind.color.id].</li>")
		if(A.forensic_holder)
			var/datum/forensic_group/basic_list/sleuth_color/sleuth = A.forensic_holder.get_group(FORENSIC_GROUP_SLEUTH_COLOR)
			if(sleuth)
				sleuth_text += sleuth.get_sleuth_text(A)
		if(!sleuth_text)
			sleuth_text = SPAN_NOTICE("Smells like \a [A], alright.")
		boutput(src.mob, SPAN_NOTICE(sleuth_text))
