...
/atom/proc/calc_material_value(var/propID, var/minVal, var/maxVal, var/defaultVal = -INFINITY, var/default_matID = null)
	if(!src.material && defaultVal != -INFINITY)
		return defaultVal
	var/datum/material_property/property = null
	...
	if(src.material)
		...
		if(defaultVal != -INFINITY)
			...
			prop_material = default_mat.getProperty(propID)
		else
