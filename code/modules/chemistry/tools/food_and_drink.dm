	...
	proc/take_a_bite(var/mob/consumer, var/mob/feeder)
		...
		if (ishuman(consumer))
			...
			if (H.bioHolder && !isvampiricany(H))
				var/datum/forensic_data/dna/dna_evid = new(H.bioHolder.dna_signature, DNA_FORM_SALIVA)
				src.add_evidence(dna_evid, FORENSIC_GROUP_DNA)
			if(H.organHolder?.head?.bite_mark)
				var/datum/forensic_data/basic/bite_data = H.organHolder.head.get_bite_evidence()
				src.add_evidence(bite_data, FORENSIC_GROUP_DAMAGE)
		src.heal(consumer)
...
/obj/item/reagent_containers/food/drinks
	...
	proc/take_a_drink(var/mob/consumer, var/mob/feeder)
		...
		if (consumer.bioHolder && !isvampiricany(consumer))
			var/datum/forensic_data/dna/dna_evid = new(consumer.bioHolder.dna_signature, DNA_FORM_SALIVA)
			src.add_evidence(dna_evid, FORENSIC_GROUP_DNA)
...
/datum/action/bar/icon/chug
	...
	onEnd()
		...
		if (glass.reagents.total_volume) //Take a sip
			...
			eat_twitch(target)
			if (target.bioHolder && !isvampiricany(target))
				var/datum/forensic_data/dna/dna_evid = new(target.bioHolder.dna_signature, DNA_FORM_SALIVA)
				glass.add_evidence(dna_evid, FORENSIC_GROUP_DNA)
