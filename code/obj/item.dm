...
/obj/item/proc/Eat(var/mob/M as mob, var/mob/user, var/by_matter_eater=FALSE, var/force_edible = FALSE)
	...
	if (M == user)
		...
		if (M.bioHolder && !isvampiricany(M))
...
/obj/item/proc/forcefeed(mob/M, mob/user, edibility_override)
	...
	SPAWN(1 SECOND)
		...
		if (!istype(src, /obj/item/reagent_containers/food) && isliving(user))
			var/mob/living/L = M
			if(L.bioHolder && !isvampiricany(L))
...
/obj/item/proc/equipped(var/mob/user, var/slot)
	...
	// Not sure if this is the right place to put this - LorrMaster
	if(ishuman(user))
		var/mob/living/carbon/human/H = user
		if(slot == SLOT_HEAD || slot == SLOT_EARS || slot == SLOT_WEAR_SUIT || slot == SLOT_W_UNIFORM || slot == SLOT_BACK)
